...
if (jUrl.indexOf('functionId=isvObfuscator') != -1) {
  ...
  if (
    ...
    console.log(
      `clientVersion=${clientVersion}|openudid=${openudid}|sign=${reqSign}|st=${reqSt}|sv=${reqSv}`,
    );
    $.signBody = `clientVersion=${clientVersion}|openudid=${openudid}|sign=${reqSign}|st=${reqSt}|sv=${reqSv}`;
  } else {
    console.log(
      `getShopHomeActivityInfo上一步抓取信息不完整，结束本次signbody 抓取`,
    );
  }
}

var notifyText = `/env ISV_SHOP_ID="${$.getdata(
  'isvShopId',
)}"\n/env ISV_VENDER_ID="${$.getdata(
  'isvVnderId',
